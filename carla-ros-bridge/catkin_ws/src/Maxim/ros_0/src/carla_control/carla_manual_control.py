...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud):
        ...
        self.throttle_pub = rospy.Publisher('acceleratorServo', Float32, queue_size=1)
        self.break_pub = rospy.Publisher('breakServo', Float32, queue_size=1)
        self.steer_pub = rospy.Publisher('steeringServo', Float32, queue_size=1)
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        throttle_angle =  self._control.throttle * 135 #Kind of makes sense but the maximum is 130 degrees lets say random guess :) #Ill have to change the zeros
        break_angle = self._control.brake * 135
        steer_angle = ( self._control.steer * -135 ) + 95 #here we state that 0 is the 135 degrees so anything below tends towards 0 and viceversa


        #self.throttle_pub.publish(throttle_angle)
        self.break_pub.publish(break_angle)
